'use client'


import React from "react";

import {
  Button,
  Dialog,
  Avatar,
} from "@material-tailwind/react";



import { ThemeProvider } from "@material-tailwind/react";
export { ThemeProvider, Button };


 
export default function home() {
  const [open, setOpen] = React.useState(false);

  const [open2, setOpen2] = React.useState(false);
 
  const handleOpen = () => setOpen(!open);

  const handleOpen2 = () => setOpen2(!open2);           
 
  return (
    <>

      <Dialog open={open} handler={handleOpen}>
      <section className="bg-center bg-no-repeat bg-red-100">
          <div className="px-4 mx-auto max-w-screen-xl text-center py-20 lg:py-56">
              <h1 className="mb-4 text-3xl font-extrabold tracking-tight leading-none text-red-400 md:text-5xl lg:text-6xl">Nosso grupo do whatsapp</h1>
       


                  <a href="https://wa.me/5513982300404"  className="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg ">
                      <Avatar
                        src="https://www.cdnlogo.com/logos/w/99/whatsapp.svg"
                        alt="avatar"
                        variant="rounded"
                        withBorder={true}
                        color="green"
                        className="p-0.5"
                        width={300}
                      />
                   
                  </a>


        
          </div>

          <div className="px-4 mx-auto max-w-screen-xl text-center py-10 lg:py-20 ">
              <h1 className="mb-4 text-3xl font-extrabold tracking-tight leading-none text-red-400 md:text-5xl lg:text-6xl">Instagram</h1>
       

                  <a href="https://www.instagram.com/bellecler?igsh=MTB0anBjMHlvYm9zYg%3D%3D&utm_source=qr"  className="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg  focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-900">
                      <Avatar
                        src="data:image/png;base64, "
                        alt="avatar"
                        variant="rounded"
                        withBorder={true}
                        color="green"
                        className="p-0.5"
                        width={100}
                      />
                   
                  </a>

        
          </div>


      </section>

      <section className="bg-center bg-no-repeat bg-red-100">
            <div className="px-4 mx-auto max-w-screen-xl text-center py-20 lg:py-56">
                                    
                  <a href="#" onClick={handleOpen} className="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-gray-700 hover:bg-gray-800 ">
                      VOLTAR
                      <svg className="w-3.5 h-3.5 ms-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                          <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9"/>
                      </svg>
                  </a>
            </div>
      </section>

      </Dialog>

      <Dialog open={open2} handler={handleOpen2}>
      <section className="bg-center bg-no-repeat bg-red-100  ">
          <div className="px-10 mx-auto max-w-screen-xl text-center py-24 lg:py-56">
              <h1 className="mb-4 text-3xl font-medium tracking-tight leading-none text-red-400 md:text-5xl lg:text-6xl">“Mas os que esperam no senhor renovam as suas forças, voam alto como águias, correm e nunca se afadigam”</h1>
       

                  <a href="#" onClick={handleOpen2} className="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-gray-700 hover:bg-gray-800 ">
                      VOLTAR
                      <svg className="w-3.5 h-3.5 ms-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                          <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9"/>
                      </svg>
                  </a>
        
          </div>                                                            
      </section>
      </Dialog>



      <section className="bg-center bg-no-repeat bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/conference.jpg')] bg-gray-700 bg-blend-multiply">
          <div className="px-4 mx-auto max-w-screen-xl text-center py-24 lg:py-56">
              <h1 className="mb-4 text-4xl font-extrabold tracking-tight leading-none text-white md:text-5xl lg:text-6xl">GRUPO DE CRESCIMENTO PARA MULHERES</h1>
              <p className="mb-8 text-lg font-normal text-gray-300 lg:text-xl sm:px-16 lg:px-48">ORAÇÃO E PALAVRA</p>
              <div className="flex flex-col space-y-4 sm:flex-row sm:justify-center sm:space-y-0">
                  <a href="#" onClick={handleOpen} className="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-900">
                      GRUPO 
                      <svg className="w-3.5 h-3.5 ms-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                          <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9"/>
                      </svg>
                  </a>
                  <a href="#" onClick={handleOpen2} className="inline-flex justify-center hover:text-gray-900 items-center py-3 px-5 sm:ms-4 text-base font-medium text-center text-white rounded-lg border border-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-400">
                      ISAÍAS 40:31
                  </a>  
              </div>
          </div>
      </section>



    </>
  );
}
